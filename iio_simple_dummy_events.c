...
{
	struct iio_dummy_eventgen *evgen = container_of(work, struct iio_dummy_evgen, work);
	struct iio_dev *indio_dev = evgen->
	struct iio_dummy_state *st = iio_priv(indio_dev);
...
int iio_simple_dummy_events_register(struct iio_dev *indio_dev)
{
	...
	if (!st->regs)
		return -EPERM;
	/* Same handler for all event interrupts */
	iio_evgen_register_work_handler(&iio_simple_dummy_event_handler);
