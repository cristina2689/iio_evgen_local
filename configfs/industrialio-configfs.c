					       ...
					       const char *name)
{
	...
	struct iio_dev_configfs *configfs_dev = container_of(group);
	configfs_dev->ops->probe(?);
	// TODO
}
...
// Configfs Group Operations ops structure
static struct configfs_group_operations device_ops = {
	.make_group = &device_make_group, // when a new group is created
...
// Group type
static struct config_item iio_devices_group_type = {
	.ct_owner = THIS_MODULE,
};
