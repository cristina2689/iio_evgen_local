 ...
 * @work:	irq work for handling interrupt generation
...
{
	if (iio_evgen)
		init_irq_work(&iio_evgen->work, f);
}
EXPORT_SYMBOL_GPL(iio_evgen_register_work_handler);
			      ...
			      size_t len)
{
	...
	if (ret)
		return ret;
	/* problem - lose first interrupt - works if mapping between reg_id and reg_data */
	iio_evgen->regs[this_attr->address].reg_id   = this_attr->address;
	iio_evgen->regs[this_attr->address].reg_data = event;

	/* trigger interrupt */
	if (iio_evgen->regs[this_attr->address].in_use)
